...
import { uploadCompanyDocument, deleteCompanyDocument } from "@/lib/users";
...
export default function CompanyDocumentUpload(
  props: CompanyDocumentUploadProps
) {
  ...
  async function handleUpload() {
    ...
    setUploading(true);
    try {
      await uploadCompanyDocument(props.uid, selectedFile, props.documentType);
  ...
  async function handleDelete() {
    ...
    setUploading(true);
    try {
      await deleteCompanyDocument(props.uid, props.documentType);
      alert("Supprimé");
      props.onUploadSuccess();
    } catch (err) {
      alert("Erreur");
  ...
  const title = labels[props.documentType];
